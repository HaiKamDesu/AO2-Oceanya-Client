...
| "music effect toggles", "Fade Out Previous", "Fade In", "Synchronize", "/play music" | `OceanyaClient/MainWindow.xaml.cs` (`MusicContextMenu_Opened`, `PlayMusicItemAsync`) + `OceanyaClient/Features/Viewport/AO2ViewportAudioManager.cs`; AO2 effect flags are user-controllable only for `SERVER LIST` rows that send direct `MC#`. `LOCAL FILES`, `FREQUENTLY USED`, and `CUSTOM COMMANDS` use `/play` or arbitrary OOC commands, so the menu disables effect toggles for those selected rows. | Confirmed |
