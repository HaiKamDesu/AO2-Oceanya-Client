...
| "taskbar preview", "viewport Windows preview", "viewport in taskbar", "Use viewport as Windows preview" | Right-click context menu on `AO2ViewportWindowContent` (XAML + `ViewportContextMenu_Opened` / `MenuItemViewportAsPreview_Click`); `UseAsWindowsPreview` bool property fires `UseAsWindowsPreviewChanged`; saved to `SaveFile.Data.GMViewportWindowPreviewPriority`; `MainWindow.ApplyViewportTaskbarPriority()` swaps `ShowInTaskbar` between viewport and host window so the viewport becomes the live taskbar preview. `ViewportThumbnailCompositor.cs` is dormant (DWM approach abandoned). | Confirmed |
