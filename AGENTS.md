...
| "IC packet parity", "MS# packet", "Attorney Online Vidya cannot talk IC" | Outgoing IC is built in `AOClient.SendICMessage` and serialized by `ICMessage.GetCommand(...)`. AO2 outbound order is core 15 fields, then `CCCC_IC_SUPPORT` adds only `showname`, `other_charid`, `self_offset`, `immediate`; `LOOPING_SFX`, `ADDITIVE`, `EFFECTS`, and `CUSTOM_BLIPS` append their own blocks independently. Do not send receive-only echo fields (`OtherName`, `OtherEmote`, `OtherOffset`, `OtherFlip`) in outbound `MS#`. AO2 IC showname textbox is not the OOC/client name: keep `AOClient.ICShowname` empty unless the user/agent explicitly sets it, and `ResolveShowNameForPacket()` should fall back to the current displayed/local character (`CurrentINI`) before the selected server INIPuppet. Server-selected `iniPuppetID` must come from `PV#<player>#CID#<charId>#%` confirmation after `CC#`; do not trust requested/snapshot char IDs. Tests: `UnitTests/CustomUnitTests.cs` `GetCommand_CcccWithEffectsOnly_DoesNotInsertReceiveOnlyOrLoopingFields`; `UnitTests/NetworkTests.cs` `ShowName_BlankShowname_FallsBackToCurrentCharacterShowname`, `SelectIniPuppet_DoesNotOverwriteIcShowname`, `HandleMessage_PvUpdatesConfirmedIniPuppet`, `SendICMessage_CcccEffectsOnly_EmitsAo2FeatureGatedPacket`. | Confirmed |
...
| "GM snapshot restore", "restored clients", "stale viewport background", "wrong INIPuppet after reconnect" | `OceanyaClient/MainWindow.xaml.cs` owns `CaptureGmMultiClientSnapshot`, `RestoreGmMultiClientSnapshotAsync`, `ApplySnapshotStateToClient`, and `ResolveSnapshotRequestedPuppet`. `Common/SaveFile.cs` `GmMultiClientSnapshot` stores `ServerEndpoint`/`ServerName`; restore only trusts saved puppet/local-character divergence on the same endpoint and never reapplies saved `Background` to live clients. `AOBot-Testing/Agents/AOClient.cs` reconnect uses `RestoreIniPuppetAfterReconnectAsync` so the old local character/emote is not paired with a newly auto-selected server INIPuppet. Tests: `UnitTests/TestabilityHardeningTests.cs` snapshot endpoint/puppet cases and `UnitTests/CustomUnitTests.cs` compact outgoing AO2 `MS#` layout. | Confirmed |
...
| "update old Oceanya client", "will savefile migrate", "user deleted old folder", "unit tests wiped production savefile" | `Common/SaveFile.cs` + `OceanyaClient/Components/Forms/InitialConfigurationWindow.xaml.cs` + `Documentation/SaveFileAndUpdatePersistence.md`; v6.1 and v6.2 use the same `%APPDATA%/OceanyaClient/savefile.json` path. Debug/dev launches use `%APPDATA%/OceanyaClientDev/savefile.json`; NUnit/testhost runs default to temp `OceanyaClientUnitTests`; `--test-savefile=...` is honored before first load. If saved `ConfigIniPath` points into a deleted release folder, startup checks the current app folder for `config.ini`, `<same parent>/config.ini`, and `base/config.ini`. Save-load diagnostics go beside the active savefile; unreadable saves are copied to `savefile.unreadable.<timestamp>.json` before falling back to defaults. | Confirmed |
