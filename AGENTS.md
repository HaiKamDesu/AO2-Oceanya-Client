...
| "find in log", "search IC/OOC log", "ctrl+f log" | Right-click on IC or OOC log → "Find in log..." → opens `FindInLogWindow` (modeless, `OceanyaWindowContentControl`). `ICLog` and `OOCLog` implement `ILogFindTarget`; `MainWindow.GetCurrentLogFindTargets()` searches both current documents. Search semantics live in `LogDocumentSearch`, which indexes the full visible `FlowDocument` text before mapping matches back to `TextPointer`s, so case/whole-word/regex work across formatted runs. `HighlightMatches(...)` marks all matches and updates only old/current active highlights on navigation; `ClearHighlight` restores via `DependencyProperty.UnsetValue`. Tests: `UnitTests/LogDocumentSearchTests.cs`. | Confirmed |
