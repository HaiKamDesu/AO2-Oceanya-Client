...
| "update old Oceanya client", "will savefile migrate", "user deleted old folder" | `Common/SaveFile.cs` + `Documentation/SaveFileAndUpdatePersistence.md`; normal releases use `%APPDATA%/OceanyaClient/savefile.json`, so replacing the release folder preserves settings unless AppData is deleted or `ConfigIniPath` points to a deleted AO folder. | Confirmed |
...
| "viewport lag", "background loading slow", "character sprite freeze", "prefetch", "disk animation cache" | All viewport asset loads are synchronous. `OnBackgroundChanged` kicks off `AO2ViewportAssetResolver.PrefetchBackground` on a background task that decodes every position variant (bg + desk) in parallel so the cache is warm before any IC message arrives. Per-slot `CancellationTokenSource` in `backgroundPrefetchCts` cancels on BG change. Also triggered on `AttachClient`. Decoded frames are persisted to `OceanyaAnimationDiskCache` (`%LocalAppData%/Oceanya/AnimCache/`): after the first decode they survive restarts as raw BGRA blobs keyed by `SHA256(path)+lastWriteTicks+maxDim`. During asset refresh (`ClientAssetRefreshService.RefreshCharactersAndBackgroundsAsync`) all background positions are pre-baked via `PrebakeAllBackgroundAnimations` so subsequent sessions are instant. Writes are synchronous on background threads (prefetch/refresh) and fire-and-forget on the UI thread. GIF files use `BitmapFrameAnimationPlayer` first (disk-cacheable) when `usePreviewLimits=false`; `GifAnimationPlayer` is kept as fallback. | Confirmed |
