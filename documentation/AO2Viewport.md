...
- `OceanyaClient/Components/Forms/SettingsWindow.xaml(.cs)`: shared dark settings dialog with sidebar pages for audio, client toggles, selected AO2 `config.ini` values, and callwords. It persists Oceanya-only rules in `savefile.json` and writes supported AO2 values back to the active `config.ini`.
...
- Callword rules live in `SaveFile.Data.CallwordRules`. A matching incoming message plays the configured custom SFX path, or falls back to AO2-style `word_call` SFX resolution. Extra audio rules live in `SaveFile.Data.ExtraAudioRules`; current support applies saved blip volume modifiers by character, showname, player, blip token, or any-message match.
