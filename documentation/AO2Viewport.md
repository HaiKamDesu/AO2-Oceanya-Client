...
In single-internal-client GM mode, selecting another profile does not repaint the viewport to a blank/background-only state when the underlying message source is unchanged. The viewport keeps the last received AO2 scene until a new server-echoed IC packet or background event changes it.
