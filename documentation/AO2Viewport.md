...
- The GM viewport button is a show/hide toggle. `SaveFile.Data.GMViewportWindowState.IsVisible` records the last visible state, so the viewport opens automatically on the next GM multi-client launch when it was left shown.
- Callword rules live in `SaveFile.Data.CallwordRules`. The settings window imports existing `config.ini` `callwords` values as default-SFX rules when they are not already in the savefile. A matching incoming message plays the configured custom SFX path, or falls back to AO2-style `word_call` SFX resolution. Extra audio rules live in `SaveFile.Data.ExtraAudioRules`; current UI supports blip volume modifiers by any message, local character folder, local showname, or local blip token.
