...
Viewport images rendered from WebP/APNG assets are currently decoded through two resize passes:
1. Magick (inside `TryDecodeAnimationFramesWithMagick`) caps the decoded frame at `MaxAnimatedPreviewDimension = 360` (see `Ao2AnimationPreview.cs`) — originally a UI preview thumbnail safeguard.
2. WPF `Image` scales the 360px decode down to the 256x192 display canvas.
...
**Attempted fix — do not retry without reading this first.** Bypassing the 360px cap by passing `maxDimension: 0` causes corrupt pixel data at native resolution: `GetPixels().ToByteArray(PixelMapping.BGRA)` returns garbage when no `Resize` call has normalized Magick's internal pixel format. The always-resize workaround (resizing even when `targetWidth == sourceWidth && targetHeight == sourceHeight`) does not fix this because Magick elides same-dimension resizes as a no-op. The broken state manifested as a brown square BG and invisible overlays.

**Correct approach (not yet implemented).** Thread the actual display target dimensions (`256 × 192` or the subregion the image fills) through `SetAnimatedImage` and `LoadImage` in `AO2ViewportControl.xaml.cs` so that Magick decodes directly to the final display size, then there is no WPF second-pass scale. That requires adding a `targetWidth`/`targetHeight` parameter chain from the call sites all the way into `TryDecodeAnimationFramesWithMagick`. The 360px preview cap should be kept for the UI preview thumbnail code paths; only the viewport rendering path should bypass it.
